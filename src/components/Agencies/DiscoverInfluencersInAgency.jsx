...
// import { getInfluencers } from '../../services/agencies/EnlistInfluencers';
// import {
//   Search,
//   Filter,
//   Instagram,
...
// export default DiscoverInfluencersInAgency;
      ...
      <div className="bg-white border-b p-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
          ...
          <div className="flex space-x-4">
            <button
              onClick={() => setCompareMode(!compareMode)}
              className={`px-4 py-2 rounded-lg flex items-center space-x-2 ${compareMode ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600'
                }`}
            >
              <BarChart className="w-5 h-5" />
                  ...
                  <div className="space-y-2">
                    {platforms.map((platform) => (
                      <button
                        key={platform.name}
                        onClick={() => togglePlatform(platform.name)}
                        className={`w-full flex items-center justify-between p-2 rounded-lg ${selectedPlatforms.includes(platform.name)
                            ? 'bg-blue-50 text-blue-600'
                            : 'bg-gray-50 text-gray-600'
                          }`}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {niches.map((niche) => (
                      <button
                        key={niche}
                        onClick={() => toggleNiche(niche)}
                        className={`p-2 rounded-lg text-sm ${selectedNiches.includes(niche)
                            ? 'bg-blue-50 text-blue-600'
                            : 'bg-gray-50 text-gray-600'
                          }`}
                      >
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between">
                      ...
                      {compareMode && (
                        <button
                          onClick={() => toggleInfluencerComparison(influencer.id)}
                          className={`p-2 rounded-lg ${selectedForComparison.includes(influencer.id)
                              ? 'bg-blue-500 text-white'
                              : 'bg-gray-100 text-gray-600'
                            }`}
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {influencer.packages.map((pkg) => (
                <div
                  ...
                  className="bg-white border rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col"
                >
                  <div className={`p-4 sm:p-6 rounded-t-xl ${pkg.package_type === 'premium'
                      ? 'bg-gradient-to-r from-purple-500 to-purple-600'
                      : pkg.package_type === 'standard'
                        ? 'bg-gradient-to-r from-blue-500 to-blue-600'
                        : 'bg-gradient-to-r from-gray-500 to-gray-600'
